...
function seed() {
    ...
    const existingVehicles = db.prepare("SELECT COUNT(*) as count FROM vehicles").get();
    if (existingVehicles.count > 0) {
        console.log("⚠️  Database already seeded — skipping.");
        return;
    }
    const insertVehicle = db.prepare("INSERT INTO vehicles (make, model, year) VALUES (@make, @model, @year)");
    const insertJob = db.prepare("INSERT INTO repair_jobs (vehicle_id, description, cost) VALUES (@vehicleId, @description, @cost)");
